...
class HwpTableTools:
    ...
    def merge_cells(
        ...
        try:
            ...
            if self.hwp_controller.merge_table_cells(
                start_row, start_col, end_row, end_col
            ):
                logger.info(
                    f"셀 병합 완료: ({start_row},{start_col}) - ({end_row},{end_col})"
                )
    ...
    def create_table_with_data(
        self, rows: int, cols: int, data: Optional[str] = None, has_header: bool = False
    ) -> str:
        """
        ...
        try:
            ...
            if data:
                try:
                    ...
                    if self.hwp_controller.fill_table_with_data(
                        str_data_array, 1, 1, has_header
                    ):
                    ...
                    logger.error(
                        f"표 데이터 입력 중 오류: {str(data_error)}", exc_info=True
                    )
                    return (
                        f"표는 생성되었으나 데이터 입력 중 오류 발생: {str(data_error)}"
                    )
    ...
    def fill_table_with_data(
        ...
        try:
            ...
            logger.info(
                f"Filling table with data: {len(data_list)} rows, starting at ({start_row}, {start_col})"
            )
            ...
            # fill_table_with_data 메서드를 사용하여 데이터 채우기
            success = self.hwp_controller.fill_table_with_data(
